# Unreal Engine 5: Modular Lab (Blueprint)
#### Personal Time Project

A blueprint system lab made in a game engine by using Unreal Engine 5.5.4.

## 🕹️ Project Overview
This project contains a lists of modular system that can be used in video game projects or game jams. Most would be prototype-quality, however, I tend to planned improved how to use Unreal Engine 5.

The project purpose to have a catalogue of systems for my portfolio while improving and understanding more aspects of Blueprints. I am still currently learning both C++ (whether using on Game Engines or using only entirely on C++ with the used of libraries) and Blueprint for Unreal Engine with a potential to utilised both for game projects.

## 💻 Technical Specs
* **Engine:** Unreal Engine 5
* **Language:** Blueprint Visual Scripting
* **Tools:** GitHub
